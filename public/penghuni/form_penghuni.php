...
$penghuni = new PenghuniKost();
$conn = $penghuni->getConnection();
$user_id = $_SESSION['user']['id'];
$message = "";

// Ambil data awal penghuni (untuk ditampilkan di form)
$sql = "SELECT nama, nomor_kamar, durasi_sewa FROM penghuni WHERE user_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $nama = $_POST['nama'];
    $nomor_kamar = $_POST['kamar'];
    $durasi_sewa = $_POST['durasi'];

    if ($penghuni->editPenghuni($user_id, $nama, $nomor_kamar, $durasi_sewa, false)) {
        $message = "Data penghuni berhasil disimpan!";
        // Refresh data
        $data = ['nama' => $nama, 'nomor_kamar' => $nomor_kamar, 'durasi_sewa' => $durasi_sewa];
    } else {
    ...
    <div class="container">
        ...
        <form method="post"> 
            ...
            <div>
               ...
               <input type="text" name="kamar" id="kamar" value="<?= htmlspecialchars($data['nomor_kamar'] ?? '') ?>" required>
            </div>
            <div>
                <label for="durasi">Durasi Sewa: </label>
                 <input type="text" name="durasi" id="durasi" value="<?= htmlspecialchars($data['durasi_sewa'] ?? '') ?>" required>
            </div>
